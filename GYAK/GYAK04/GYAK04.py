...
#data_frame = dict_to_dataframe(stats)
...
#print(type(get_column(data_frame, 'area')))
...
# %%
def get_top_two(df:pd.DataFrame)-> pd.core.series.Series:
    new_df = df.copy()
    return new_df.nlargest(n=2, columns='area')

#print(type(get_top_two(data_frame)))
...
# %%
def population_density(df:pd.DataFrame)-> pd.core.frame.DataFrame:
    new_df = df.copy()
    density = new_df['population'] / new_df['area']
    new_df['density'] = density
    return new_df
#print(type(population_density(data_frame)))
...
#fig = plot_population(data_frame)
